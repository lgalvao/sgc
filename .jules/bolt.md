...
**Learning:** `sgc.alerta` was missing indexes on `unidade_destino_codigo` and `processo_codigo`, causing full table scans for common dashboard queries. `sgc.alerta_usuario` was missing an index on `usuario_titulo`.
**Action:** Always verify `schema.sql` against `JpaRepository` queries (especially `findBy...`) to ensure foreign keys and filter columns are indexed.
