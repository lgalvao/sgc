...
COMMENT ON COLUMN FEEDBACK.metadata_json IS 'Metadados contextuais do sistema e navegador em formato JSON.';
COMMENT ON COLUMN FEEDBACK.caminho_screenshot IS 'Caminho no sistema de arquivos para a captura de tela.';
COMMENT ON COLUMN FEEDBACK.usuario_id IS 'Identificador do usuario que enviou o feedback.';
COMMENT ON COLUMN FEEDBACK.usuario_nome IS 'Nome do usuario que enviou o feedback.';
COMMENT ON COLUMN FEEDBACK.enviado_em IS 'Data e hora do envio do feedback.';
COMMENT ON COLUMN FEEDBACK.rota IS 'Caminho da rota/pagina onde o feedback foi gerado.';
COMMENT ON COLUMN FEEDBACK.status IS 'Situacao do feedback: NOVO, REVISADO, RESOLVIDO, DESCARTADO.';
...
BEGIN
    ...
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX idx_feedback_status ON FEEDBACK (status)';
    END IF;
END;
/
...
BEGIN
    ...
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX idx_feedback_usuario ON FEEDBACK (usuario_id)';
    END IF;
END;
/
...
BEGIN
    ...
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX idx_feedback_data ON FEEDBACK (enviado_em)';
    END IF;
END;
/
