...
ALTER TABLE ALERTA
    MODIFY (processo_codigo NULL);
ALTER TABLE ALERTA
    MODIFY (unidade_destino_codigo NULL);

COMMENT ON COLUMN ALERTA.usuario_destino_titulo IS 'Usuario destino do alerta para alertas pessoais.';

ALTER TABLE ALERTA
    ADD CONSTRAINT ck_alerta_destino CHECK (
...
COMMENT ON TABLE NOTIFICACAO_EMAIL IS 'Caixa de saída de e-mails, para envio assíncrono com retry e auditoria.';
...
COMMENT ON COLUMN NOTIFICACAO_EMAIL.data_hora_criacao IS 'Data/hora de criação do registro de outbox.';
COMMENT ON COLUMN NOTIFICACAO_EMAIL.data_hora_envio IS 'Data/hora do envio bem-sucedido.';
COMMENT ON COLUMN NOTIFICACAO_EMAIL.ultimo_erro IS 'Último erro registrado durante tentativa de envio.';
