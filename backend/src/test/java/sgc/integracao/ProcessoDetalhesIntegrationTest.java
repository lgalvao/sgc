...
/**
 * Testes de integração focados no endpoint /api/processos/{id}.
 * Este endpoint é chamado pelo ProcessoView para mostrar a lista de unidades
 * participantes.
 */
...
class ProcessoDetalhesIntegrationTest extends BaseIntegrationTest {
        ...
        void setUp() {
                ...
                var subprocessoEmAndamento = new Subprocesso(processoEmAndamento, unidade, mapaEmAndamento,
                ...
                var subprocessoFinalizado = new Subprocesso(processoFinalizado, unidade, mapaFinalizado,
        ...
        class DetalhesProcesso {
                ...
                void deveRetornarDetalhesComUnidades_QuandoProcessoEmAndamento() throws Exception {
                        var result = mockMvc.perform(get(API_PROCESSO_DETALHES, processoEmAndamento.getCodigo()))
                ...
                void deveRetornarDetalhesComUnidades_QuandoProcessoFinalizado() throws Exception {
                        var result = mockMvc.perform(get(API_PROCESSO_DETALHES, processoFinalizado.getCodigo()))
                                        .andExpect(status().isOk())
                                        .andReturn();

                        System.out.println("Resposta JSON (finalizado): " + result.getResponse().getContentAsString());
                ...
                void deveRetornar404_QuandoProcessoNaoExiste() throws Exception {
                        mockMvc.perform(get(API_PROCESSO_DETALHES, 999999L))
