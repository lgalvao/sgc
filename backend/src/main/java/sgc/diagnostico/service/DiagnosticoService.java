...
public class DiagnosticoService {
    ...
    public DiagnosticoDto buscarDiagnosticoCompleto(Long subprocessoCodigo) {
        ...
        List<ServidorDiagnosticoDto> servidoresDto = servidores.stream()
                .map(servidor -> {
                    List<AvaliacaoServidor> avaliacoes = avaliacoesPorServidor.getOrDefault(servidor.getTituloEleitoral(), List.of());
                    List<OcupacaoCritica> ocupacoes = ocupacoesPorServidor.getOrDefault(servidor.getTituloEleitoral(), List.of());
    ...
    public Diagnostico buscarOuCriarDiagnosticoEntidade(Long subprocessoCodigo) {
        return diagnosticoRepo.findBySubprocessoCodigo(subprocessoCodigo)
                .orElseGet(() -> {
                    Subprocesso subprocesso = subprocessoRepo.findById(subprocessoCodigo)
                            .orElseThrow(() -> new EntityNotFoundException("Subprocesso não encontrado: " + subprocessoCodigo));
                    
                    // Validação extra: verificar se é tipo DIAGNOSTICO? 
                    // Por flexibilidade, vamos assumir que quem chama sabe o que está fazendo,
                    // mas num cenário real validaríamos o TipoProcesso.
    ...
    @Transactional
    public List<AvaliacaoServidorDto> buscarMinhasAvaliacoes(Long subprocessoCodigo, String servidorTitulo) {
        Diagnostico diagnostico = buscarOuCriarDiagnosticoEntidade(subprocessoCodigo);
        return avaliacaoServidorRepo.findByDiagnosticoCodigoAndServidorTituloEleitoral(
    ...
    @Transactional
    public void concluirAutoavaliacao(Long subprocessoCodigo, String servidorTitulo, ConcluirAutoavaliacaoRequest request) {
        Diagnostico diagnostico = buscarOuCriarDiagnosticoEntidade(subprocessoCodigo);
        ...
        if (avaliacoes.size() < totalCompetencias) {
            throw new IllegalStateException("Não é possível concluir autoavaliação: existem competências não avaliadas.");
        }
        ...
        // Nota: A modelagem atual coloca a situação EM CADA avaliação. 
        // Idealmente seria uma entidade ServidorNoDiagnostico.
        // Como simplificação, atualizamos todas as avaliações para refletir o status do servidor.
        avaliacoes.forEach(a -> 
            a.setSituacao(SituacaoServidorDiagnostico.AUTOAVALIACAO_CONCLUIDA)
        );
    ...
    public DiagnosticoDto concluirDiagnostico(Long subprocessoCodigo, ConcluirDiagnosticoRequest request) {
        ...
        // Valida pendências
        // Vamos recalcular DTO para ter visão atualizada
        ...
        long servidoresPendentes = overview.servidores().stream()
                .filter(s -> !s.situacao().equals(SituacaoServidorDiagnostico.AUTOAVALIACAO_CONCLUIDA.name())
                        && !s.situacao().equals(SituacaoServidorDiagnostico.CONSENSO_APROVADO.name())
                        && !s.situacao().equals(SituacaoServidorDiagnostico.AVALIACAO_IMPOSSIBILITADA.name()))
                .count();
        ...
        // Valida ocupações críticas? CDU-09 diz "Ocupações críticas preenchidas"
        // Considerando que ocupação crítica deve ser preenchida para todos ou só para quem tem GAP?
        // DRAFT não especifica regra rígida de 'todas'. Vamos aceitar se o chefe diz que terminou.
        ...
        return overview; // Retorna status anterior ou atualizado? Melhor retornar atualizado
        // Mas para simplificar retornamos o overview com a nova situação no controller se precisar
    }

    private boolean validarSePodeConcluirDiagnostico(List<ServidorDiagnosticoDto> servidores) {
        // Regra básica: todos servidores devem ter concluído ou estarem impossibilitados
        return servidores.stream()
                .allMatch(s -> 
                        SituacaoServidorDiagnostico.AUTOAVALIACAO_CONCLUIDA.name().equals(s.situacao()) ||
                        SituacaoServidorDiagnostico.CONSENSO_APROVADO.name().equals(s.situacao()) ||
                        SituacaoServidorDiagnostico.AVALIACAO_IMPOSSIBILITADA.name().equals(s.situacao())
                );
    }
}
