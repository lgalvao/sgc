...
import sgc.service.ProcessoService;
...
public class ProcessoController {
    ...
    @PostMapping
    public ResponseEntity<ProcessoDTO> criarProcesso(@Valid @RequestBody CreateProcessRequest request) {
        ProcessoDTO criado = processoService.create(request);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProcessoDTO> atualizarProcesso(@PathVariable Long id, @Valid @RequestBody UpdateProcessRequest request) {
        try {
            ProcessoDTO atualizado = processoService.update(id, request);
    ...
    public ResponseEntity<sgc.dto.ProcessDetailDTO> detalhesProcesso(
            @PathVariable Long id,
            @RequestParam(name = "perfil") String perfil,
            @RequestParam(name = "unidade", required = false) Long unidade) {
        try {
            sgc.dto.ProcessDetailDTO detail = processoService.getDetails(id, perfil, unidade);
    ...
    public ResponseEntity<ProcessoDTO> iniciarProcesso(
        ...
        try {
            ...
            if ("REVISAO".equalsIgnoreCase(tipo)) {
                ...
                resultado = processoService.startMappingProcess(id, unidades);
            }
