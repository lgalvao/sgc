...
public class SubprocessoMapaService {
  ...
  public void salvarAjustesMapa(Long codSubprocesso, List<CompetenciaAjusteDto> competencias,
    ...
    for (CompetenciaAjusteDto compDto : competencias) {
      var competencia = competenciaRepo.findById(compDto.getCodCompetencia())
          .orElseThrow(() -> new ErroEntidadeNaoEncontrada("Competência não encontrada: %d"
          .formatted(compDto.getCodCompetencia())));
  ...
  public void importarAtividades(Long codSubprocessoDestino, Long codSubprocessoOrigem) {
    final Subprocesso spDestino = subprocessoRepo.findById(codSubprocessoDestino)
    ...
    Subprocesso spOrigem = subprocessoRepo.findById(codSubprocessoOrigem)
        .orElseThrow(() -> new ErroEntidadeNaoEncontrada("Subprocesso de origem não encontrado: %d"
    ...
    for (Atividade atividadeOrigem : atividadesOrigem) {
      ...
      List<Conhecimento> conhecimentosOrigem = conhecimentoRepo
          .findByAtividadeCodigo(atividadeOrigem.getCodigo());
    ...
    movimentacaoRepo.save(new Movimentacao(spDestino, unidadeOrigem, spDestino.getUnidade(),
        descMovimentacao, null));
