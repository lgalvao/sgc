...
public class SgrhService {
    ...
    public Optional<UsuarioDto> buscarUsuarioPorTitulo(String titulo) {
        ...
        return Optional.of(new UsuarioDto(titulo, "Usuário Mock " + titulo, titulo + "@tre-pe.jus.br", "MAT" + titulo.substring(0, Math.min(6, titulo.length())), "Analista Judiciário"));
    }
    ...
    // ... (restante dos métodos mockados originais)
    public Optional<UsuarioDto> buscarUsuarioPorEmail(String email) {
        String titulo = email.split("@")[0];
        return Optional.of(new UsuarioDto(titulo, "Usuário " + titulo, email, "MAT" + titulo.hashCode(), "Analista Judiciário"));
    }
    ...
    public Map<String, UsuarioDto> buscarUsuariosPorTitulos(List<String> titulos) {
        ...
        return titulos.stream().collect(Collectors.toMap(titulo -> titulo, titulo -> new UsuarioDto(titulo, "Usuário Mock " + titulo, titulo + "@tre-pe.jus.br", "MAT" + titulo.substring(0, Math.min(6, titulo.length())), "Analista Judiciário"), (u1, u2) -> u1));
    }
