...
O módulo utiliza o padrão **Service Facade**. O `MapaService` atua como uma fachada única para todos os clientes, sejam eles internos (como `MapaController`) ou externos (como `ProcessoService`). Ele orquestra os serviços especializados para executar tanto operações de CRUD quanto lógicas de negócio mais complexas.
...
- **`CopiaMapaService`**: Responsável por clonar um mapa existente (deep copy), incluindo suas competências, atividades e conhecimentos. É utilizado pelo `ProcessoService` ao iniciar ciclos de revisão.
...
- **`MapaIntegridadeService`**: Contém regras de validação para garantir a integridade de um mapa (ex: regras de associação).
