...
public class ImpactoMapaServiceImpl implements ImpactoMapaService {
    ...
    public ImpactoMapaDto verificarImpactos(Long subprocessoId) {
        ...
        List<AtividadeImpactadaDto> inseridas = detectarAtividadesInseridas(atividadesAtuais, atividadesVigentes);
        List<AtividadeImpactadaDto> removidas = detectarAtividadesRemovidas(atividadesAtuais, atividadesVigentes, mapaVigente);
        List<AtividadeImpactadaDto> alteradas = detectarAtividadesAlteradas(atividadesAtuais, atividadesVigentes, mapaVigente);
    ...
    /**
     * Detecta atividades que foram inseridas (estão no atual, não estavam no vigente).
     * Critério: comparação por descrição (case-insensitive).
     */
    private List<AtividadeImpactadaDto> detectarAtividadesInseridas(List<Atividade> atuais, List<Atividade> vigentes) {
    ...
    /**
     * Detecta atividades que foram removidas (estavam no vigente, não estão no atual).
     * Critério: comparação por descrição (case-insensitive).
     */
    ...
    private List<CompetenciaImpactadaDto> identificarCompetenciasImpactadas(
                ...
                if (comp != null && comp.getMapa().getCodigo().equals(mapaVigente.getCodigo())) {
                    CompetenciaImpactoAcumulador acumulador = impactosMap
                            .computeIfAbsent(comp.getCodigo(),
                                    k -> new CompetenciaImpactoAcumulador(comp.getCodigo(), comp.getDescricao()));
        ...
        for (AtividadeImpactadaDto atividade : alteradas) {
            List<CompetenciaAtividade> vinculos = competenciaAtividadeRepository.findByAtividadeCodigo(atividade.codigo());
            for (CompetenciaAtividade vinculo : vinculos) {
                Competencia comp = competenciaRepository
                        .findById(vinculo.getId().getCompetenciaCodigo())
                        .orElse(null);
