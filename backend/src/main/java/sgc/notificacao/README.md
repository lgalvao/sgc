...
pelo `ApplicationEventPublisher` do Spring e, em resposta, delega as tarefas para os módulos `notificacao` e `alerta`.
...
- **`EventoProcessoListener`**: O principal ponto de entrada reativo da aplicação. Ouve os eventos de domínio e
  orquestra as ações de notificação e alerta.
...
- **Desacoplamento:** O `ProcessoService` não sabe como as notificações ou alertas são tratados. Ele apenas anuncia
  que "algo aconteceu".
