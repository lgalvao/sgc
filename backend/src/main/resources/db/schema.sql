...
(
    ...
    situacao_servidor_anterior varchar(50) check (situacao_servidor_anterior in ('AUTOAVALIACAO_NAO_INICIADA', 'AUTOAVALIACAO_CONCLUIDA', 'CONSENSO_CRIADO', 'CONSENSO_APROVADO', 'AVALIACAO_IMPOSSIBILITADA')),
...
(
    ...
    constraint fk_situacao_capacitacao_diagnostico foreign key (diagnostico_codigo) references sgc.diagnostico,
    constraint fk_situacao_capacitacao_servidor foreign key (servidor_titulo) references sgc.vw_usuario,
    constraint fk_situacao_capacitacao_competencia foreign key (competencia_codigo) references sgc.competencia,
