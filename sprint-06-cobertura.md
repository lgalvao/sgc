...
## Objetivo
Medir a cobertura de testes para evitar regressões futuras.

## Tarefas
- Integrar plugin JaCoCo no build (Gradle).
- Configurar publicação do relatório no CI.
- Definir um gate inicial brando (para não quebrar o build imediatamente, mas monitorar).

## Critérios de Aceite
- Relatórios de cobertura são gerados com sucesso (`./gradlew jacocoTestReport`).
