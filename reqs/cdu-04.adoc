...
. No Painel, ADMIN clica em um processo de mapeamento que esteja na situação 'Criado'.

. O sistema muda para a tela `Cadastro de processo`, com os campos preenchidos com as informações do processo
selecionado.

. ADMIN clica no botão Iniciar processo.

. O sistema mostra modal de confirmação: "Ao iniciar o processo, não será mais possível editá-lo ou removê-lo e todas
as unidades participantes serão notificadas por e-mail.", com botões `Confirmar` e `Cancelar`.

. Caso o usuário escolha o botão `Cancelar`, o sistema interrompe a operação de iniciação do processo, permanecendo na
mesma tela.
...
. O sistema muda a situação do processo de mapeamento para 'Em andamento';

. O sistema cria internamente um subprocesso para cada unidade participante, que seja do tipo Operacional ou
Interoperacional,
...
. O sistema cria internamente um mapa de competências vazio (sem competências) e o vincula ao subprocesso da unidade.
...
.. Para cada unidade intermediária será criado um alerta com:
