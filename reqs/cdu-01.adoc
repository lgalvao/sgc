...
. O usuário informa suas credenciais: número do título de eleitor e senha

. O sistema verifica título/senha através da API do Sistema Acesso do TRE-PE

. Caso o usuário não seja autenticado com as credenciais fornecidas, sistema mostra a mensagem
`Título ou senha inválidos.`

. Caso o usuário seja autenticado, o sistema consulta perfis e unidades do usuário nas views conectadas ao ba.
Importante: Um usuário pode estar
em várias unidades (ex. substituição, atribuição temporária) e também ter mais de um perfil.

. O sistema determina os perfis disponíveis para o usuário, seguindo estas regras, não exclusivas:
...
. O sistema determina quais pares 'perfil-unidade' se aplicam ao usuário logado, seguindo estas regras:
...
.. O sistema expande a tela de login para permitir a seleção de perfil/unidade

.. O usuário seleciona o perfil/unidade com o qual vai atuar

.. O sistema guarda o perfil e a unidade definidos

. O sistema exibe a estrutura de telas da aplicação, composta pelas seções: `Barra de navegação`, `Conteúdo` e
`Rodapé`.

.. `A Barra de navegação` é sempre mostrada no topo das telas (exceto para tela de login) e tem as seguintes
regras de exibição:
...
... Itens adicionais, alinhados à direita:
...
.. O `Conteúdo` compõe a parte central onde serão 'encaixadas' todas as telas.

.. O `Rodapé` é a seção localizada na parte inferior das telas (exceto na tela de login), em que aparecerá a
informação da versão do sistema assim como a informação `Desenvolvido por SESEL/COSIS/TRE-PE`.

. O sistema mostra inicialmente a tela `Painel`.
