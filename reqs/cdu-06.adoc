...
. O sistema mostra a tela Detalhes do processo com os dados do processo escolhido.
. A tela será composta pelas seções Dados do processo e Unidades participantes.
.. Seção `Dados do processo` (sem título):
... Informações da descrição, tipo e da situação dos processos (ver arquivo _situacoes.md).
... Se for perfil ADMIN, exibe o botão Finalizar processo.
.. Seção Unidades participantes:
...
... Caso existam unidades subordinadas cujos subprocessos estejam localizados na unidade do usuário, os seguintes
