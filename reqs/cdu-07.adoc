...
. O sistema mostra a tela Detalhes do subprocesso com os dados do subprocesso da unidade do perfil.
. A tela Detalhes do subprocesso será composta por três seções: Dados da Unidade, Movimentações do Processo e Elementos
do Processo.

.. Na seção Dados da unidade serão apresentadas as informações:

... (Sem título): Sigla e nome da unidade, destacado.
...
... Situação do Subprocesso: Informação descritiva da situação do subprocesso da unidade (ver seção Situações de
subprocessos).
... Localização atual: Unidade destino da última movimentação do subprocesso da unidade.
... Prazo para conclusão da etapa atual: Data limite da última etapa do subprocesso ainda não concluída na
unidade.

.. Na seção Movimentações do processo, título "Movimentações", é apresentada uma tabela com as movimentações que o
...
.. Na seção Elementos do processo (sem título) serão apresentados cards clicáveis com informação variável em função
do tipo do processo, os quais darão acesso às telas específicas de cada tema.

... Se o processo for dos tipos Mapeamento ou Revisão, a seção apresentará cards para acesso ao cadastro de
...
... Se o processo for do tipo Diagnóstico, a seção apresentará estes cards:
