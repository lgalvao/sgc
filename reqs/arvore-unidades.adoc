...
. *Marcado (checked)*: Todas as filhas estão marcadas (recursivamente)
. *Desmarcado (unchecked)*: Nenhuma filha está marcada
. *Indeterminado (indeterminate)*: Algumas filhas marcadas, mas não todas
...
. *NÃO é INTERMEDIARIA* (unidades INTERMEDIARIA nunca são elegíveis)
. *NÃO está participando de outro processo ativo*
. Para REVISAO/DIAGNOSTICO: *possui mapa vigente*
...
. Usuário clica em COORD_11 (INTERMEDIARIA)
. Todas as filhas elegíveis são marcadas (SECAO_111, SECAO_112, SECAO_113)
. COORD_11 fica *marcada* (todas filhas marcadas)
. Ao salvar, apenas as SECAOs são enviadas ao backend

=== Cenário 2: Selecionar Parcialmente

. Usuário marca SECAO_111 e SECAO_112
. COORD_11 fica *indeterminada* (nem todas filhas marcadas)
. Ao salvar, apenas SECAO_111 e SECAO_112 são enviadas

=== Cenário 3: Desmarcar Coordenadoria

. COORD_11 está marcada (todas filhas marcadas)
. Usuário clica em COORD_11 para desmarcar
. Todas as filhas são desmarcadas
. COORD_11 fica *desmarcada*

=== Cenário 4: Unidade com Filhas Não Elegíveis

. COORD_11 tem 3 filhas, mas apenas 1 é elegível (tem mapa vigente)
. COORD_11 está *habilitada* (tem pelo menos uma filha elegível)
. Usuário clica em COORD_11
. Apenas a filha elegível é marcada
. COORD_11 fica *indeterminada* (nem todas filhas marcadas)
