...
- **Verbos HTTP:**
    - `GET`: Para consultas.
    - `POST`: Para criação e, por convenção deste projeto, também para **atualização** e **exclusão** (usando sufixos na
      URL, ex: `/api/recurso/{id}/atualizar`, `/api/recurso/{id}/excluir`).
...
- **Componentes:** Devem ser "burros" (apresentacionais), recebendo dados via `props` e emitindo eventos via `emits`.
  Consulte `frontend/src/components/README.md`.
- **Views:** Responsáveis por conectar as Stores aos Componentes.

### 4.2. Tecnologias

- **UI:** Utilize componentes da biblioteca **BootstrapVueNext** (`BButton`, `BModal`, etc.) em vez de HTML/Bootstrap
