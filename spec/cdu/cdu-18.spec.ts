...
test.describe('CDU-18: Visualizar mapa de competências', () => {
  ...
  test('deve visualizar mapa como GESTOR', async ({ page }) => {
    ...
    // Verificar se há algum elemento com data-testid
    const allTestIds = page.locator('[data-testid]');
    const count = await allTestIds.count();
  ...
  test('deve mostrar competências e atividades associadas', async ({ page }) => {
    ...
    if (await atividades.count() > 0) {
      ...
      const conhecimentos = primeiraAtividade.locator('[data-testid="conhecimento-item"]');
