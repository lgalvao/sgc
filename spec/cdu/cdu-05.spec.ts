...
test.describe('CDU-05: Iniciar processo de revisão', () => {
  test('deve iniciar processo de revisão', async ({ page }) => {
    ...
    // Preencher formulário para processo de revisão
    await page.getByLabel('Descrição').fill('Processo de Revisão Teste');
    await page.getByLabel('Tipo').selectOption('Revisão');
