...
test.describe('CDU-09: Disponibilizar cadastro de atividades e conhecimentos', () => {
  test('deve disponibilizar cadastro com sucesso', async ({ page }) => {
    ...
    // Navegar para subprocesso em andamento
    ...
    await expect(page.getByRole('heading', { name: 'Disponibilização do cadastro' })).toBeVisible();
    await expect(page.getByText('Confirma a finalização e a disponibilização do cadastro?')).toBeVisible();

    // Confirmar a ação
    await page.getByRole('button', { name: 'Confirmar' }).click();

    // Verificar mensagem de sucesso
    await expect(page.getByText('Cadastro de atividades disponibilizado')).toBeVisible();

    // Verificar redirecionamento para painel
    await page.waitForURL('**/painel');
  });

  test('deve mostrar erro quando atividade não tem conhecimento associado', async ({ page }) => {
    // Login como CHEFE
    await loginAsChefe(page);
    ...
    await page.getByLabel('Nova atividade').fill('Atividade sem conhecimento');
  ...
  test('deve cancelar disponibilização', async ({ page }) => {
    ...
    await expect(page.getByText('Cadastro de atividades disponibilizado')).not.toBeVisible();
  });
});