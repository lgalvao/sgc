...
- **Exceções de Serviço:** Para erros que ocorrem na camada de serviço, é crucial que o `Controller` os trate adequadamente. Se um serviço lança uma exceção específica, como `ErroDominioNaoEncontrado`, o `Controller` deve ter um bloco `catch` para essa exceção e retornar o status HTTP apropriado (por exemplo, 404 Not Found). Se isso não for feito, um `catch` genérico de `Exception` pode mascarar o erro real, resultando em um status 500 Internal Server Error, o que dificulta a depuração e torna o teste de `status().isNotFound()` falho.
