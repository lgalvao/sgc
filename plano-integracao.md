...
*   `GET /analises-cadastro`: Lista histórico de análises de cadastro.
    *   **Entrada:** `id` (PathVariable - subprocesso)
    *   **Saída:** `List<Analise>` (entidade `Analise` diretamente)
*   `POST /analises-cadastro`: Cria nova análise de cadastro.
    *   **Entrada:** `id` (PathVariable - subprocesso), `Map<String, String> payload` (contém `observacoes`, `unidadeSigla`, `analistaUsuarioTitulo`, `motivo`)
    *   **Saída:** `Analise`
*   `GET /analises-validacao`: Lista histórico de análises de validação.
    *   **Entrada:** `id` (PathVariable - subprocesso)
    *   **Saída:** `List<Analise>`
*   `POST /analises-validacao`: Cria nova análise de validação.
    *   **Entrada:** `id` (PathVariable - subprocesso), `Map<String, String> payload`
    *   **Saída:** `Analise`
*   **DTOs:** `CriarAnaliseRequestDto`, `AnaliseHistoricoDto`, `AnaliseValidacaoHistoricoDto`.
...
*   `GET /`: Lista todos os subprocessos.
    *   **Saída:** `List<SubprocessoDto>`
*   `GET /{id}`: Obtém subprocesso por ID.
    *   **Saída:** `SubprocessoDetalheDto`
*   `POST /`: Cria novo subprocesso.
    *   **Entrada:** `SubprocessoDto`
    *   **Saída:** `SubprocessoDto`
*   `PUT /{id}`: Atualiza subprocesso.
    *   **Entrada:** `id` (PathVariable), `SubprocessoDto`
    *   **Saída:** `SubprocessoDto`
*   `DELETE /{id}`: Exclui subprocesso.
    *   **Saída:** `ResponseEntity<Void>`
*   **DTOs:** `SubprocessoDto`, `SubprocessoDetalheDto`.
...
*   `GET /{id}/mapa-completo`: Obtém mapa completo com competências e atividades aninhadas.
    *   **Saída:** `MapaCompletoDto`
...
*   **`frontend/src/mappers/alertas.ts` (novo)**: Mapear `AlertaDto` para o modelo `Alerta` do frontend.
*   **`frontend/src/mappers/analises.ts` (novo)**: Mapear `AnaliseHistoricoDto` e `AnaliseValidacaoHistoricoDto` para o modelo `AnaliseValidacao` do frontend.
*   **`frontend/src/mappers/atividades.ts` (novo)**: Mapear `AtividadeDto` e `ConhecimentoDto` para os modelos `Atividade` e `Conhecimento` do frontend.
*   **`frontend/src/mappers/competencias.ts` (novo)**: Mapear `CompetenciaDto` e `CompetenciaAtividade` para o modelo `Competencia` do frontend.
*   **`frontend/src/mappers/mapas.ts` (novo)**: Mapear `MapaDto`, `MapaCompletoDto`, `ImpactoMapaDto`, `MapaAjusteDto`, `MapaVisualizacaoDto` para o modelo `Mapa` do frontend.
*   **`frontend/src/mappers/processos.ts` (novo)**: Mapear `ProcessoDto`, `ProcessoDetalheDto`, `ProcessoResumoDto` para o modelo `Processo` do frontend.
*   **`frontend/src/mappers/subprocessos.ts` (novo)**: Mapear `SubprocessoDto`, `SubprocessoDetalheDto`, `SubprocessoCadastroDto`, `SugestoesDto` para o modelo `Subprocesso` do frontend.
*   **`frontend/src/mappers/sgrh.ts` (novo)**: Mapear `AutenticacaoRequest`, `EntrarRequest`, `PerfilUnidade`, `ResponsavelDto`, `UnidadeDto`, `UsuarioDto` para os modelos correspondentes do frontend.
...
*   **Frontend:**
    *   `frontend/src/axios-setup.ts`: Configure a instância do Axios para apontar para o backend e implemente um interceptor de requisições/respostas para tratamento global de erros (e.g., 401, 403, 500). **[CONCLUÍDO]**
    *   `frontend/src/stores/notificacoes.ts`: Utilize a `notificacoesStore` para exibir mensagens de erro amigáveis ao usuário. **[CONCLUÍDO]**
*   **Testes E2E:**
...
*   **Frontend:**
    *   **Mappers:** Crie/atualize `frontend/src/mappers/sgrh.ts` para mapear `AutenticacaoRequest`, `EntrarRequest`, `PerfilUnidade`, `UsuarioDto`. **[CONCLUÍDO]**
    *   **Serviços de API:** Crie `frontend/src/services/usuarioService.ts` com métodos para autenticar, autorizar e entrar. **[CONCLUÍDO]**
    *   **Stores Pinia:** Atualize `frontend/src/stores/perfil.ts` para chamar `usuarioService` e armazenar dados reais. **[CONCLUÍDO]**
    *   **Componentes Vue:** Modifique `frontend/src/views/Login.vue` para substituir a lógica de mock por chamadas ao `usuarioService`. **[CONCLUÍDO]**
*   **Testes E2E:**
    *   Adapte `frontend/e2e/cdu/cdu-01.spec.ts` para usar o fluxo de login real, verificando autenticação bem-sucedida, falha de credenciais e logout. **[CONCLUÍDO]**
...
*   **Frontend:**
    *   **Mappers:** Crie/atualize `frontend/src/mappers/processos.ts` (mapear `ProcessoResumoDto`), `frontend/src/mappers/alertas.ts` (mapear `AlertaDto`). **[CONCLUÍDO]**
    *   **Serviços de API:** Crie `frontend/src/services/painelService.ts` (métodos para listar processos e alertas), `frontend/src/services/alertaService.ts` (método para marcar alerta como lido). **[CONCLUÍDO]**
    *   **Stores Pinia:** Atualize `frontend/src/stores/processos.ts` e `frontend/src/stores/alertas.ts` para buscar dados reais. **[CONCLUÍDO]**
    *   **Componentes Vue:** Modifique `frontend/src/views/Painel.vue` para substituir mocks por dados das stores atualizadas. **[CONCLUÍDO]**
*   **Testes E2E:**
    *   Adapte `frontend/e2e/cdu/cdu-02.spec.ts` para verificar a listagem de processos e alertas vindos do backend, ordenação e marcação de alertas como lidos. **[CONCLUÍDO]**
...
*   **Frontend:**
    *   **Mappers:** Crie/atualize `frontend/src/mappers/processos.ts` (mapear `CriarProcessoReq`, `AtualizarProcessoReq`, `ProcessoDto`, `ProcessoDetalheDto`). **[CONCLUÍDO]**
    *   **Serviços de API:** Crie `frontend/src/services/processoService.ts` com métodos para CRUD de processos. **[CONCLUÍDO]**
...
*   **Frontend:**
    *   **Serviços de API:** Atualize `frontend/src/services/processoService.ts` com métodos para iniciar e finalizar processos.
...
*   **Frontend:**
    *   **Mappers:** Crie/atualize `frontend/src/mappers/mapas.ts` (mapear todos os DTOs de mapa).
    *   **Serviços de API:** Crie `frontend/src/services/mapaService.ts`, atualize `frontend/src/services/subprocessoService.ts` (métodos relacionados a mapa).
    *   **Stores Pinia:** Atualize `frontend/src/stores/mapas.ts` para chamar os serviços.
    *   **Componentes Vue:** Modifique `frontend/src/views/CadMapa.vue` (edição), `frontend/src/views/VisMapa.vue` (visualização e ações de workflow).
*   **Testes E2E:**
...
*   **Frontend:**
    *   **Mappers:** Crie/atualize `frontend/src/mappers/unidades.ts` e `frontend/src/mappers/servidores.ts`.
    *   **Serviços de API:** Crie `frontend/src/services/unidadeService.ts`, `frontend/src/services/servidorService.ts`, `frontend/src/services/analiseService.ts`, etc.
    *   **Stores Pinia:** Atualize `frontend/src/stores/unidades.ts`, `frontend/src/stores/servidores.ts`, `frontend/src/stores/analises.ts` e `frontend/src/stores/configuracoes.ts`.
