        ...
        AND sit_unid NOT LIKE 'E%') uni_c
         ...
         LEFT JOIN (SELECT e.datainicio,
                    ...
                    WHERE e.datainicio < TRUNC(SYSDATE) + 1
                      AND e.datatermino >= TRUNC(SYSDATE)) r
                   ON c.id = r.central_id
         LEFT JOIN corau.ct_zona z
                   ON r.zona_id = z.id
         LEFT JOIN (SELECT cd, sigla_unid_tse, num_ze
                    FROM srh2.unidade_tse
...
WITH zona_responsavel AS (SELECT codigo_central,
     ...
     lotacao_calculada AS (SELECT ld.cod_unid_tse,
     ...
     filhas_complexas AS (SELECT uf.cod_unid_super,
                          ...
                          WHERE NVL(ld.qtd_servidores, 0) > 1
                             OR EXISTS (SELECT 1
                                        ...
                                        WHERE neta.cod_unid_super = uf.cd)
                          GROUP BY uf.cod_unid_super),
     titulares AS (SELECT l.cod_unid_tse,
                          s.mat_servidor,
                          s.num_tit_ele,
                          c.dt_ingresso
...
WITH unidade AS (SELECT codigo,
                        ...
                        unidade_superior_codigo
                 FROM VW_UNIDADE),
     unidade_apoio AS (SELECT MIN(CASE WHEN sigla = 'ASPRE' AND tipo = 'OPERACIONAL' THEN codigo END) AS codigo_aspre,
...
FROM (SELECT codigo, matricula_titular, titulo_titular, data_inicio_titularidade
      ...
      WHERE situacao = 'ATIVA'
        AND tipo IN ('OPERACIONAL', 'INTEROPERACIONAL', 'INTERMEDIARIA')) u
         LEFT JOIN (SELECT sub.mat_servidor,
                    ...
                    WHERE NVL(c.titular_com, 0) = 1
                      ...
                      AND sub.dt_fim_subst >= TRUNC(SYSDATE)) s
                   ON u.matricula_titular = s.mat_servidor
         LEFT JOIN (SELECT unidade_codigo,
                           usuario_matricula,
                           usuario_titulo,
                           data_inicio,
                           data_termino
                    ...
                    WHERE data_inicio < TRUNC(SYSDATE) + 1
                      AND data_termino >= TRUNC(SYSDATE)) a
