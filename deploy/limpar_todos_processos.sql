...
WHERE alerta_codigo IN (SELECT codigo FROM sgc.alerta WHERE processo_codigo IS NOT NULL);
DELETE
FROM sgc.alerta
WHERE processo_codigo IS NOT NULL;
...
INTO v_count
FROM user_tables
WHERE table_name = 'AVALIACAO_SERVIDOR';
IF
v_count > 0 THEN
...
INTO v_count
FROM user_tables
WHERE table_name = 'SITUACAO_CAPACITACAO';
IF
v_count > 0 THEN
...
DELETE
FROM sgc.unidade_mapa; -- Limpa o vínculo de "mapa vigente" das unidades
DELETE
FROM sgc.atividade;
DELETE
