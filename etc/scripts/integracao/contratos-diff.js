...
const CAMINHO_OPENAPI_DIFF = resolverNaRaiz("etc/scripts/node_modules/.bin/openapi-diff");
const CAMINHO_RELATORIO_MD = resolverNaRaiz("etc/qualidade/openapi/latest/diff-openapi.md");
...
async function main() {
    ...
    if (args.includes("--help") || args.includes("-h")) {
        exibirAjudaComando({
            ...
            exemplos: [
                "node etc/scripts/sgc.js integracao contratos diff",
                "node etc/scripts/sgc.js integracao contratos diff --anterior /tmp/old.json --atual /tmp/new.json",
                "node etc/scripts/sgc.js integracao contratos diff --json"
            ]
        });
        return;
    }
