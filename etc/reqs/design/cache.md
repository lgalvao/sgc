...
O SGC é uma SPA com rotas em `keepAlive`. Quando o usuário navega entre telas,
componentes preservados em memória são **reativados** via `onActivated` em vez de
remontados. Para evitar consultas desnecessárias ao backend a cada reativação,
...
`store.invalidar()` em resposta a ações do usuário. Composables e views
**não devem importar stores de cache diretamente apenas para invalidar**.

Isso garante rastreabilidade: ao buscar quem invalida o painel ou o processo,
basta procurar por `useInvalidacaoNavegacao`.
...
`carregarContextoInicial`) sincronizam o estado local a partir de dados já
carregados do backend. Não alteram nada no servidor — são operações de leitura
e **não devem invalidar** caches de outras telas.

Apenas **ações de escrita confirmadas pelo backend** (salvar, importar,
disponibilizar, homologar, devolver, alterar data…) devem disparar invalidação.
...
O painel exibe dados de `ProcessoResumo`. Use `incluirPainel: true` sempre que
a mutação alterar **qualquer um destes campos**:
...
Use `useInvalidacaoNavegacao` diretamente dentro do composable — a dependência
fica explícita e rastreável.
...
Se o usuário permanece na tela atual após a ação (ex: ações de bloco que ficam
no `ProcessoDetalheView`), não há razão para invalidar o painel agora. Quando
o usuário eventualmente navegar para o painel, o TTL de 5 minutos cobre a
