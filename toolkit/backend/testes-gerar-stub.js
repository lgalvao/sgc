...
async function main() {
    ...
    if (args.includes('--help') || args.includes('-h')) {
        exibirAjudaComando({
            ...
            exemplos: [
                'node toolkit/sgc.js backend testes gerar-stub ProcessoService',
                'node toolkit/sgc.js backend testes gerar-stub sgc.processo.service.ProcessoService'
            ]
        });
        process.exit(0);
    }
    ...
    const deps = [];
    srcContent.split('\n').forEach(l => {
        const m = l.match(/private\s+final\s+([\w<>]+)\s+(\w+);/);
        if (m) deps.push({type: m[1], name: m[2]});
    });
    ...
    if (!fs.existsSync(testPath)) {
        ...
        console.log('⚠️ Arquivo existente: ' + testPath);
    }
}
