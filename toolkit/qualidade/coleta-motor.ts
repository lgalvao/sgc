...
async function executarColeta(
    ...
    const adaptadoresDoPerfil = opcoes.perfis[perfilInformado];
    if (!adaptadoresDoPerfil) {
        throw new Error(`Perfil de qualidade invalido: ${perfilInformado}`);
    }
    const adaptadoresAusentes = adaptadoresDoPerfil.filter(nomeAdaptador => !opcoes.adaptadores[nomeAdaptador]);
    ...
    for (const nomeAdaptador of adaptadoresDoPerfil) {
        const adaptador = opcoes.adaptadores[nomeAdaptador];
        if (!adaptador) {
