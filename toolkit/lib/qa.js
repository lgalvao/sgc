...
const CAMINHO_SNAPSHOT_LATEST = resolverNaRaiz("toolkit", "qa-dashboard", "latest", "ultimo-snapshot.json");
...
async function resolverSnapshotQa(caminhoInformado = null) {
    ...
    const candidatos = await globby("toolkit/qa-dashboard/runs/**/snapshot.json", {
        cwd: resolverNaRaiz(),
        absolute: true,
        onlyFiles: true
    });
    ...
    if (!maisRecente) {
        throw new Error("Nenhum snapshot de QA foi encontrado. Execute `node toolkit/sgc.js qa snapshot coletar --perfil rapido`.");
    }
...
async function carregarModuloQaDashboard() {
    const caminho = resolverNaRaiz("toolkit", "qa-dashboard", "dashboard.html");
    return {
