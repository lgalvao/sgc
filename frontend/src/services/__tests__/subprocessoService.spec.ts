...
vi.mock('@/utils/apiError', () => ({
    normalizarErro: vi.fn(() => ({kind: 'inesperado', message: 'Erro'}))
}));
...
describe('subprocessoService', () => {
    ...
    it('disponibilizarMapa', async () => {
        ...
        expect(apiClient.post).toHaveBeenCalledWith('/subprocessos/1/disponibilizar-mapa', {
    ...
    it('atualizarCompetencia', async () => {
        ...
        expect(apiClient.post).toHaveBeenCalledWith('/subprocessos/1/competencia/2', {
            descricao: 'desc',
            atividadesCodigos: [1]
        });
    });
