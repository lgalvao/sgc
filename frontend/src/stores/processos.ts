...
// Funções de mapeamento e parsing de datas (mantidas por enquanto para subprocessos)
...
function parseSubprocessoDates(pu: Partial<Subprocesso>): Subprocesso {
  return {
    ...
    dataLimiteEtapa1:
      typeof pu.dataLimiteEtapa1 === 'string' ? parseDate(pu.dataLimiteEtapa1) || new Date() : new Date(),
    dataFimEtapa1: typeof pu.dataFimEtapa1 === 'string' ? parseDate(pu.dataFimEtapa1) : null,
    dataLimiteEtapa2:
      typeof pu.dataLimiteEtapa2 === 'string' ? parseDate(pu.dataLimiteEtapa2) : null,
...
// DTO do backend para Processo
interface ProcessoDto {
  id: number
  descricao: string
  tipo: string // "MAPEAMENTO", "REVISAO", etc.
  situacao: string // "CRIADO", "EM_ANDAMENTO", etc.
  dataLimite: string // "YYYY-MM-DD"
  dataFinalizacao: string | null // "YYYY-MM-DD"
}
  ...
  getters: {
    ...
    getMovementsForSubprocesso: (state) => (idSubprocesso: number) => {
      return state.movements
        .filter((m) => m.idSubprocesso === idSubprocesso)
        .sort((a, b) => b.dataHora.getTime() - a.dataHora.getTime())
    }
  },
  actions: {
    /**
     * Carrega os processos da API.
     */
    async carregarProcessos() {
      if (this.processos.length > 0) return
    ...
    // Ações existentes (adicionar, remover, editar, etc.)
    // Elas precisarão ser atualizadas para chamar a API no futuro.
    // Por enquanto, elas continuarão a modificar o estado local.
    ...
    async alterarDataLimiteSubprocesso(payload: {
      ...
      if (subprocessoIndex !== -1) {
        ...
        this.addMovement({
          ...
          unidadeOrigem: 'SEDOC',
          unidadeDestino: 'SEDOC',
          descricao: `Data limite da etapa ${etapa} alterada para ${
            novaDataLimite.toISOString().split('T')[0]
          }`
        })
        ...
        notificacoesStore.email(
          ...
          `Prezado(a) responsável pela ${unidade},\n\nA data limite da etapa ${etapa} no processo foi alterada para ${
            novaDataLimite.toISOString().split('T')[0]
    ...
    async aceitarMapa(payload: { idProcesso: number; unidade: string; perfil: string }) {
    ...
    async rejeitarMapa(payload: { idProcesso: number; unidade: string }) {
    ...
    async apresentarSugestoes(payload: { idProcesso: number; unidade: string; sugestoes: string }) {
    ...
    async validarMapa(payload: { idProcesso: number; unidade: string }) {
      const { idProcesso, unidade } = payload
      const unidadesStore = useUnidadesStore()
