...
2. **Reset**: Implemente um método `$reset` ou uma action `limparEstado` se a store precisar ser resetada entre
   navegações.
3. **Persistência**: Dados sensíveis de sessão (token, usuário) são persistidos no `localStorage` para sobreviver ao
   refresh (F5).
