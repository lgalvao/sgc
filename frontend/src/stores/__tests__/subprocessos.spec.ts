...
describe('Subprocessos Store', () => {
    ...
    it('fetchSubprocessoDetalhe deve carregar detalhes com sucesso', async () => {
        ...
        perfilStore.perfisUnidades = [
            { perfil: Perfil.GESTOR, unidade: { codigo: 99, sigla: 'TEST', nome: 'Teste' }, siglaUnidade: 'TEST' }
        ];
        ...
        expect(subprocessoService.fetchSubprocessoDetalhe).toHaveBeenCalledWith(123, 'GESTOR', 99);
    ...
    it('fetchSubprocessoDetalhe deve notificar erro se perfil não selecionado', async () => {
        ...
        expect(notificacoesStore.erro).toHaveBeenCalledWith('Erro ao buscar detalhes do subprocesso', 'Informações de perfil ou unidade não disponíveis.');
    ...
    it('fetchSubprocessoDetalhe deve tratar erro do serviço', async () => {
        ...
        vi.mocked(subprocessoService.fetchSubprocessoDetalhe).mockRejectedValue(new Error('Fail'));

        await store.fetchSubprocessoDetalhe(123);

        expect(notificacoesStore.erro).toHaveBeenCalledWith('Erro ao buscar detalhes do subprocesso', 'Não foi possível carregar as informações.');
    });

    it('fetchSubprocessoPorProcessoEUnidade deve retornar código com sucesso', async () => {
        const store = useSubprocessosStore();
        vi.mocked(subprocessoService.buscarSubprocessoPorProcessoEUnidade).mockResolvedValue({ codigo: 555 } as any);

        const result = await store.fetchSubprocessoPorProcessoEUnidade(1, 'UNID');

        expect(result).toBe(555);
        expect(subprocessoService.buscarSubprocessoPorProcessoEUnidade).toHaveBeenCalledWith(1, 'UNID');
    });
    ...
    it('fetchSubprocessoPorProcessoEUnidade deve tratar erro', async () => {
        ...
        const notificacoesStore = useNotificacoesStore();
        vi.spyOn(notificacoesStore, 'erro');
        vi.mocked(subprocessoService.buscarSubprocessoPorProcessoEUnidade).mockRejectedValue(new Error('Fail'));

        const result = await store.fetchSubprocessoPorProcessoEUnidade(1, 'UNID');

        expect(result).toBeNull();
        expect(notificacoesStore.erro).toHaveBeenCalledWith('Erro', 'Não foi possível encontrar o subprocesso para esta unidade.');
    });
    ...
    it('alterarDataLimiteSubprocesso deve delegar para processosStore', async () => {
        ...
        vi.spyOn(processosStore, 'alterarDataLimiteSubprocesso').mockResolvedValue(undefined);

        await store.alterarDataLimiteSubprocesso(1, { novaData: '2023-01-01' });

        expect(processosStore.alterarDataLimiteSubprocesso).toHaveBeenCalledWith(1, { novaData: '2023-01-01' });
    });
    ...
    it('disponibilizarCadastro deve executar ação com sucesso e atualizar processo', async () => {
        ...
        vi.spyOn(processosStore, 'fetchProcessoDetalhe').mockResolvedValue(undefined);
        ...
        expect(notificacoesStore.sucesso).toHaveBeenCalledWith('Cadastro disponibilizado', 'Cadastro disponibilizado.');
    ...
    it('disponibilizarCadastro deve tratar erro', async () => {
        ...
        vi.mocked(cadastroService.disponibilizarCadastro).mockRejectedValue(new Error('Fail'));

        const success = await store.disponibilizarCadastro(123);

        expect(success).toBe(false);
        expect(notificacoesStore.erro).toHaveBeenCalledWith('Erro ao disponibilizar', 'Não foi possível concluir a ação: Erro ao disponibilizar.');
    });
