...
describe("ImpactoMapaModal.vue", () => {
  ...
  it("deve mostrar atividades alteradas", async () => {
    const impacto = {
      ...
      atividadesInseridas: [],
      atividadesRemovidas: [],
      atividadesAlteradas: [{ codigo: 3, descricao: "Atividade Nova Desc", descricaoAnterior: "Atividade Velha Desc" }],
      competenciasImpactadas: [],
    };
  ...
  it("deve emitir evento 'fechar' ao clicar no botão", async () => {
      const impacto = { temImpactos: false, atividadesInseridas: [], atividadesRemovidas: [], atividadesAlteradas: [], competenciasImpactadas: [] };
      wrapper = createWrapper({}, { mapas: { impactoMapa: impacto } });
      await flushPromises();
  ...
  it("deve exibir estado de carregamento", async () => {
      ...
      mapasStore.buscarImpactoMapa = vi.fn(() => new Promise(() => {})); // Promise que nunca resolve
