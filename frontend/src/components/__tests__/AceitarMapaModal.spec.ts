...
describe("AceitarMapaModal.vue", () => {
    ...
    it("deve renderizar o modal em modo aceite por padrão", () => {
        const wrapper = createWrapper({homologacao: false});
    ...
    it("deve renderizar o modal em modo homologação", () => {
        const wrapper = createWrapper({homologacao: true});
    ...
    it("deve emitir o evento fecharModal ao clicar no botão de cancelar", async () => {
        ...
        await wrapper
            ...
            .trigger("click");
        expect(wrapper.emitted("fecharModal")).toHaveLength(1);
    });
    ...
    it("deve emitir o evento confirmarAceitacao com a observação", async () => {
        ...
        await wrapper.find('[data-testid="inp-aceite-mapa-observacao"]').setValue("Observação teste");
