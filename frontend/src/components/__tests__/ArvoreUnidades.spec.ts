...
describe('ArvoreUnidades - Seleção Hierárquica', () => {
  ...
  function encontrarUnidade(codigo: number, unidadesArray: Unidade[]): Unidade | null {
  ...
  function processarSelecao(selecionadasAnteriores: number[], selecionadasNovas: number[]): number[] {
    const adicionadas = selecionadasNovas.filter(c => !selecionadasAnteriores.includes(c));
    const removidas = selecionadasAnteriores.filter(c => !selecionadasNovas.includes(c));
  ...
  describe('Cenários complexos', () => {
    ...
    it('deve manter hierarquia ao marcar/desmarcar níveis intermediários', () => {
      ...
      resultado = processarSelecao(resultado, resultado.filter(c => c !== 10));
  ...
  describe('Unidades INTEROPERACIONAIS', () => {
    ...
    it('CAS pode ficar marcada sem suas filhas (característica INTEROPERACIONAL)', () => {
      ...
      resultado = processarSelecao(resultado, resultado.filter(c => c !== 202));
    ...
    it('STIC também pode ficar marcada sem todas as filhas', () => {
      ...
      // Desmarcar SEDIA (9)
      resultado = processarSelecao(resultado, resultado.filter(c => c !== 9));
