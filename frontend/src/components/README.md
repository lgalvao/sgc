...
### `comum/`
Componentes utilitários e compartilhados (ex: `ErrorAlert.vue`, `InlineEditor.vue`, `LoadingButton.vue`, `EmptyState.vue`, `ModalConfirmacao.vue`).

### `configuracoes/`
Componentes para as telas de configurações do sistema.

### `layout/`
Componentes de estrutura e navegação da página (ex: `LayoutPadrao.vue`, `PageHeader.vue`, `MainNavbar.vue`, `BarraNavegacao.vue`).
