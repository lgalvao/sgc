  ...
  <div class="campo-usuario">
    ...
    <div v-if="mostrarResultadosUsuarios && termoPesquisaMinimaAtingida" class="usuario-dropdown overflow-auto"
      ...
      <BListGroup v-else-if="usuariosEncontrados.length > 0" flush>
        <BListGroupItem
            v-for="(u, i) in usuariosEncontrados" :id="`opcao-usuario-${u.tituloEleitoral}`" :key="u.tituloEleitoral"
            :active="indiceUsuarioDestacado === i" :data-testid="`opcao-usuario-${u.tituloEleitoral}`"
            action button
            @mousedown.prevent="selecionarUsuario(u)"
