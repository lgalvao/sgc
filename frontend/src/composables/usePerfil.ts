...
export function usePerfil() {
  ...
  const getPerfisDoServidor = (idServidor: number): PerfilUnidade[] => {
    ...
    unidadesChefiadas.forEach(unidade => {
      pares.push({ perfil: getPerfilDaUnidade(unidade), unidade: unidade, siglaUnidade: unidade.sigla });
    });
    ...
    if (unidadePrincipal) {
      ...
      const hasNonServidorProfileForPrincipalUnit = pares.some(
        p => p.unidade.sigla === unidadePrincipal.sigla && p.perfil !== Perfil.SERVIDOR,
      );

      if (isOperacional && !hasNonServidorProfileForPrincipalUnit) {
        pares.push({ perfil: Perfil.SERVIDOR, unidade: unidadePrincipal, siglaUnidade: unidadePrincipal.sigla });
      }
    }

    // 4. Remove duplicatas exatas (mesmo perfil, mesma unidade)
    return pares.filter((par, index, self) =>
      index === self.findIndex(p => p.perfil === par.perfil && p.unidade.sigla === par.unidade.sigla),
    );
  };
