...
vi.mock('../views/PainelView.vue', () => ({default: {template: '<div>Painel</div>'}}));
vi.mock('../views/CadProcesso.vue', () => ({default: {template: '<div>CadProcesso</div>'}}));
vi.mock('../views/ProcessoView.vue', () => ({default: {template: '<div>Processo</div>'}}));
vi.mock('../views/SubprocessoView.vue', () => ({default: {template: '<div>Subprocesso</div>'}}));
vi.mock('../views/CadMapa.vue', () => ({default: {template: '<div>CadMapa</div>'}}));
vi.mock('../views/VisMapa.vue', () => ({default: {template: '<div>VisMapa</div>'}}));
vi.mock('../views/CadAtividades.vue', () => ({default: {template: '<div>CadAtividades</div>'}}));
vi.mock('../views/VisAtividades.vue', () => ({default: {template: '<div>VisAtividades</div>'}}));
vi.mock('../views/DiagnosticoEquipe.vue', () => ({default: {template: '<div>DiagnosticoEquipe</div>'}}));
vi.mock('../views/OcupacoesCriticas.vue', () => ({default: {template: '<div>OcupacoesCriticas</div>'}}));
vi.mock('../views/UnidadeView.vue', () => ({default: {template: '<div>Unidade</div>'}}));
vi.mock('../views/CadAtribuicao.vue', () => ({default: {template: '<div>CadAtribuicao</div>'}}));
vi.mock('../views/HistoricoView.vue', () => ({default: {template: '<div>Historico</div>'}}));
vi.mock('../views/RelatoriosView.vue', () => ({default: {template: '<div>Relatorios</div>'}}));
vi.mock('../views/ConfiguracoesView.vue', () => ({default: {template: '<div>Configuracoes</div>'}}));
...
describe('Router', () => {
    ...
    it('deve resolver a rota de SubprocessoVisCadastro com props dinâmicos', async () => {
        ...
        const matched = route.matched.find(m => m.name === 'SubprocessoVisCadastro');
        if (matched && typeof matched.props.default === 'function') {
            const props = matched.props.default(route);
            expect(props).toEqual({
                codProcesso: 123,
    ...
    it('deve resolver a rota de AtribuicaoTemporariaForm', async () => {
        ...
        const matched = route.matched.find(m => m.name === 'AtribuicaoTemporariaForm');
        if (matched && typeof matched.props.default === 'function') {
            const props = matched.props.default(route);
            expect(props).toEqual({
                sigla: 'DTI'
