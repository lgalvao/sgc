...
export function normalizarErro(erro: unknown): ErroNormalizado {
    ...
    if (ehErroAxios(erro) && erro.response) {
        ...
        return {
            ...
            codigo: p?.code, status, detalhes: p?.details, erros: p?.erros, traceId: p?.traceId,
            stackTrace: p?.stackTrace || obterStack(erro), erroOriginal: erro
        };
    }
    if (erro instanceof Error) {
        return {tipo: 'inesperado', mensagem: erro.message || 'Erro inesperado.', stackTrace: erro.stack, erroOriginal: erro};
    }
    logger.error("[normalizarErro] Erro não mapeado:", erro);
    return {tipo: 'inesperado', mensagem: 'Erro desconhecido ou não mapeado pela aplicação.', stackTrace: obterStack(erro) || String(erro), erroOriginal: erro};
}
