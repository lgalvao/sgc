...
describe('CadMapa.vue', () => {
  ...
  const mockMapaVisualizacao = {
      ...
      competencias: [{
          codigo: 10,
          atividades: mockAtividades // This mock structure might need refinement if mapper expects deeply nested
          // Mapper uses: dto.competencias.flatMap(c => c.atividades)
      }]
  }
    ...
    const wrapper = mount(CadMapa, {
      global: {
          ...
          createTestingPinia({
            ...
            initialState: {
               perfil: {
                 ...
                 perfisUnidades: [
                   { perfil: Perfil.CHEFE, unidade: { codigo: 1, sigla: 'TESTE' } }
                 ]
               },
               ...customState
            },
        ...
        stubs: {
            ...
            BCard: { name: 'BCard', template: '<div class="card"><slot /></div>' },
            BCardBody: { name: 'BCardBody', template: '<div class="card-body"><slot /></div>' },
            BFormInput: {
              name: 'BFormInput',
              props: ['modelValue'],
            ...
            BAlert: { name: 'BAlert', template: '<div role="alert"><slot /></div>' }
        },
        directives: {
            'b-tooltip': {}
        }
  ...
  beforeEach(() => {
    ...
    vi.mocked(mapaService.obterMapaCompleto).mockResolvedValue(mockMapaCompleto as any)
  ...
  it('deve abrir modal e criar nova competencia', async () => {
    ...
    expect(subprocessoService.adicionarCompetencia).toHaveBeenCalledWith(123, expect.objectContaining({
  ...
  it('deve editar uma competencia existente', async () => {
    ...
    expect(subprocessoService.atualizarCompetencia).toHaveBeenCalledWith(123, expect.objectContaining({
        codigo: 10,
  ...
  it('deve excluir uma competencia', async () => {
    ...
    const deleteModal = wrapper.findComponent('[data-testid="excluir-competencia-modal"]');
