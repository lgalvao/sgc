...
describe('CadProcesso.vue', () => {
  ...
  beforeEach(() => {
    ...
    const mockUsuario = {
      ...
      unidade: { codigo: 1, nome: 'Unidade A', sigla: 'UA' },
      email: 'test@user.com',
      ramal: '1234'
    };

    vi.mocked(mapaService).verificarMapaVigente.mockResolvedValue(true);
    vi.mocked(UsuariosService).buscarUsuariosPorUnidade.mockResolvedValue([mockUsuario]);
  ...
  describe('unidadesStatus', () => {
    ...
    it('disables units without servers for DIAGNOSTICO type', async () => {
      ...
      vi.mocked(mapaService).verificarMapaVigente.mockResolvedValue(true);
      vi.mocked(UsuariosService).buscarUsuariosPorUnidade.mockImplementation(async (codigo) => codigo === 1 ? [mockUsuario] : []);
