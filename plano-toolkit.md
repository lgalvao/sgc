...
- `npm --prefix toolkit run test`: 85 testes aprovados em 2 arquivos;
...
configuráveis do Semgrep; outra chega a 85 com filtros de cheiros dependentes da configuração. Nenhuma dessas mudanças
...
- 2 arquivos de teste concentrando 85 cenários;
...
| Resolvido parcialmente | Base externa é parcialmente ignorada | Arquitetura, resíduos, OpenAPI, coleta, Semgrep e cheiros agora respeitam a base/configuração; outros comandos ainda precisam da mesma correção. |
...
| Média | Testes não representam pacote externo | Os 85 testes rodam dentro do monorepo e encontram dependências hoisted. Não existe instalação em diretório isolado nem teste de raiz do consumidor. |
...
- Knip aprovado e 85 testes verdes são gates úteis, mas ainda insuficientes para afirmar ausência de código morto fora
...
5. **[concluído nesta rodada]** Adicionar testes focados para configuração inválida e combinação de defaults. Arquitetura,
   resíduos, OpenAPI, coleta, Semgrep e cheiros agora calculam seus defaults após a resolução de `--base`; outros
   defaults import-time continuam pendentes.
