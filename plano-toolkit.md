...
- `npm --prefix toolkit run test`: 86 testes aprovados em 2 arquivos;
...
configuráveis do Semgrep; outra chegou a 85 com filtros de cheiros dependentes da configuração; esta chega a 86 com a
auditoria de assuntos dependente de `backendCodigo`. Nenhuma dessas mudanças
...
- 2 arquivos de teste concentrando 86 cenários;
...
| Média | Testes não representam pacote externo | Os 86 testes rodam dentro do monorepo e encontram dependências hoisted. Não existe instalação em diretório isolado nem teste de raiz do consumidor. |
...
- Knip aprovado e 86 testes verdes são gates úteis, mas ainda insuficientes para afirmar ausência de código morto fora
...
5. **[concluído nesta rodada]** Adicionar testes focados para configuração inválida e combinação de defaults. Arquitetura,
   resíduos, OpenAPI, coleta, Semgrep, cheiros e assuntos de notificação agora calculam seus defaults após a resolução
   de `--base`; outros defaults import-time continuam pendentes.
