...
- `npm --prefix toolkit run test`: 92 testes aprovados em 2 arquivos;
...
esta chega a 92 com o núcleo AST de arquitetura dependente de `frontendCodigo`. Nenhuma dessas mudanças
...
- 2 arquivos de teste concentrando 92 cenários;
...
| Média | Testes não representam pacote externo | Os 92 testes rodam dentro do monorepo e encontram dependências hoisted. Não existe instalação em diretório isolado nem teste de raiz do consumidor. |
...
- Knip aprovado e 92 testes verdes são gates úteis, mas ainda insuficientes para afirmar ausência de código morto fora
