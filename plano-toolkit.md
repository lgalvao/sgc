...
- `npm --prefix toolkit run test`: 97 testes aprovados em 2 arquivos;
...
- 31 arquivos TypeScript de implementação;
- 41 arquivos JavaScript de implementação ainda pendentes;
- 2 arquivos JavaScript de teste (`test/sgc.test.js` e `test/cdus.test.js`);
- 2 arquivos de teste concentrando 97 cenários;
...
O núcleo TypeScript está adiantado, mas a migração do toolkit como um todo ainda não terminou: aproximadamente 43% dos
...
| Média | Testes não representam pacote externo | Os 97 testes rodam dentro do monorepo e encontram dependências hoisted. Não existe instalação em diretório isolado nem teste de raiz do consumidor. |
...
- Knip aprovado e 97 testes verdes são gates úteis, mas ainda insuficientes para afirmar ausência de código morto fora
...
2. **[parcial nesta rodada]** Migrar `backend/lib/testes-analisar-regras.ts`, `requisitos/cdus-mensagens-lib.ts`,
   ...
   `frontend/arquitetura-lib.ts`, `projeto/diagnostico.ts`, `projeto/limpar.ts`, `qualidade/coleta-execucao.ts`,
   `qualidade/coleta.ts`, `qualidade/resumo.ts` e os três comandos de contratos OpenAPI; ainda faltam achados de
   auditoria e comandos
...
1. **[parcial nesta rodada]** Projeto: diagnóstico e limpeza já convertidos; faltam preparação e perfil de qualidade.
