...
| Alta | Auditores gravam por padrão | Resíduos, cobertura, nomenclatura, Semgrep e diff OpenAPI ainda têm escrita automática ou defaults distintos. `codigo cheiros auditar`, `frontend arquitetura auditar` e `backend arquitetura auditar` já exigem `--gravar`; a diretriz read-only ainda precisa ser aplicada às demais famílias. |
...
| Auditoria com geração por padrão | `backend coesao/contratos auditar`, `frontend residuos auditar/validar`, `codigo nomes coletar-simbolos/auditar-consistencia/auditar-idioma`, `codigo semgrep auditar` | Gravem fotografias ou relatórios em caminhos padrão. A maioria oferece `--sem-gravar`, mas a ausência da opção ainda permite mutação de artefatos. `codigo cheiros auditar`, `frontend arquitetura auditar` e `backend arquitetura auditar` foram retirados desta classe e só gravam com `--gravar`. |
