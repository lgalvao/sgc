...
- `npm --prefix toolkit run test`: 91 testes aprovados em 2 arquivos;
...
`frontend` configurado no gate arquitetural; agora chega a 91 com a coleta de resíduos dependente de `frontendCodigo`.
...
- 2 arquivos de teste concentrando 91 cenários;
...
| Média | Testes não representam pacote externo | Os 91 testes rodam dentro do monorepo e encontram dependências hoisted. Não existe instalação em diretório isolado nem teste de raiz do consumidor. |
...
- Knip aprovado e 91 testes verdes são gates úteis, mas ainda insuficientes para afirmar ausência de código morto fora
