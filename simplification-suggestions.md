...
* **Remoção de Facades:** Camadas de Facade que atuam apenas como *pass-through* (ex: `PainelFacade`, `UsuarioFacade`) são consideradas *overengineering*. A lógica de orquestração deve ser consolidada diretamente nos Services relevantes de domínio ou de aplicação.
...
* **Remoção de Stores Pinia Pass-through:** Stores do Pinia (ex: `mapas.ts`, `atividades.ts`, `subprocessos.ts`, `processos.ts`, `usuarios.ts`) que atuam apenas como repasse de chamadas para APIs ou como cache temporário de dados de tela devem ser removidas. Em vez disso, utilize Composables padrão do Vue ou `refs` locais com chamadas diretas para os arquivos da pasta `services/`.
