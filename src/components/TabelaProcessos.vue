<template>
  <div>
    <table class="table table-hover" data-testid="tabela-processos">
      <thead>
      <tr>
        <th data-testid="coluna-descricao" style="cursor:pointer" @click="emit('ordenar', 'descricao')">
          Descrição
          <span v-if="criterioOrdenacao === 'descricao'">{{ direcaoOrdenacaoAsc ? '↑' : '↓' }}</span>
        </th>
        <th data-testid="coluna-tipo" style="cursor:pointer" @click="emit('ordenar', 'tipo')">
          Tipo
          <span v-if="criterioOrdenacao === 'tipo'">{{ direcaoOrdenacaoAsc ? '↑' : '↓' }}</span>
        </th>
        <th data-testid="coluna-unidades" style="cursor:pointer" @click="emit('ordenar', 'unidades')">
        ...
        <th data-testid="coluna-situacao" style="cursor:pointer" @click="emit('ordenar', 'situacao')">
          Situação
      ...
      <tbody>
      <tr v-for="processo in processos" :key="processo.id" class="clickable-row" style="cursor:pointer;"
        ...
        <td v-if="showDataFinalizacao">{{ processo.dataFinalizacaoFormatada }}</td>
