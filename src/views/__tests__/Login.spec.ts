...
vi.mock('vue-router', () => ({
  ...
  useRoute: vi.fn(() => ({
    params: {}, // Adicionado mock para useRoute
  })),
}));
...
describe('Login.vue', () => {
  ...
  let routerPushMock: ReturnType<typeof import('vue-router').useRouter>['push'];
  ...
  beforeEach(() => {
    ...
    unidadesStore.unidades = [
      { sigla: 'SESEL', nome: 'Seção de Sistemas Eleitorais', titular: 1, responsavel: null, tipo: 'OPERACIONAL', filhas: [] },
      { sigla: 'SEDESENV', nome: 'Seção de Desenvolvimento de Sistemas', titular: 9, responsavel: null, tipo: 'OPERACIONAL', filhas: [] },
      { sigla: 'SEDIA', nome: 'Seção de Dados de Inteligência Artificial', titular: 10, responsavel: null, tipo: 'OPERACIONAL', filhas: [] },
    ];
  ...
  it('deve renderizar corretamente os campos de login', async () => {
  ...
  it('deve exibir seletor de perfil/unidade para usuário com múltiplos perfis/unidades', async () => {
    ...
    atribuicaoTemporariaStore.atribuicoes = [
      { unidade: 'SEDIA', servidorId: 1, dataInicio: new Date(), dataTermino: new Date(), justificativa: 'Teste' },
    ];
