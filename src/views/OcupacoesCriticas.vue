  ...
  <div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div class="d-flex gap-2">
        <button class="btn btn-outline-success" @click="finalizarIdentificacao">
          <i class="bi bi-check-circle me-2"></i>Finalizar Identificação
        </button>
      </div>
    </div>
    ...
    <div class="card mb-4">
      <div class="card-header">
        <h5 class="card-title mb-0">Ocupações Críticas Identificadas</h5>
      </div>
      <div class="card-body">
        <div v-if="ocupacoesCriticas.length === 0" class="text-muted">
          Nenhuma ocupação crítica identificada ainda.
        </div>
        <div v-for="(ocupacao, index) in ocupacoesCriticas" :key="index" class="border rounded p-3 mb-3">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              <h6 class="mb-2">{{ ocupacao.nome }}</h6>
              <p class="mb-2 text-muted">{{ ocupacao.descricao }}</p>
              <div class="mb-2">
                <strong>Competências críticas associadas:</strong>
                <div class="d-flex flex-wrap gap-1 mt-1">
                  <span v-for="comp in ocupacao.competenciasCriticas" :key="comp"
                        class="badge bg-warning text-dark">{{ comp }}</span>
                </div>
              </div>
              <small class="text-muted">Nível de criticidade: {{ ocupacao.nivelCriticidade }}/5</small>
            </div>
            <button class="btn btn-sm btn-outline-danger" @click="removerOcupacao(index)">
    ...
    <div class="card">
      <div class="card-header">
        <h5 class="card-title mb-0">Adicionar Ocupação Crítica</h5>
      </div>
      <div class="card-body">
        <form @submit.prevent="adicionarOcupacao">
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input v-model="novaOcupacao.nome" type="text" class="form-control" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">Nível de Criticidade</label>
              <select v-model="novaOcupacao.nivelCriticidade" class="form-select" required>
                <option value="1">1 - Baixo</option>
                <option value="2">2 - Baixo-Médio</option>
            ...
            <div class="col-12">
              ...
              <textarea v-model="novaOcupacao.descricao" class="form-control" rows="3" required></textarea>
            </div>
            <div class="col-12">
              <label class="form-label">Competências Críticas Associadas</label>
              <div class="border rounded p-3">
                <div v-for="competencia in competencias" :key="competencia.id" class="form-check">
                  <input
                  ...
                  >
                  <label :for="'comp-' + competencia.id" class="form-check-label">
            ...
            <div class="col-12">
              <button type="submit" class="btn btn-primary">
                <i class="bi bi-plus-circle me-2"></i>Adicionar Ocupação
              </button>
            </div>
          </div>
        </form>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Finalizar Identificação</h5>
