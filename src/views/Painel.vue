  ...
  <div class="container mt-4">
    ...
    <div v-if="perfil === 'GESTOR' || perfil.value === 'SEDOC'" class="mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <router-link v-if="perfil.value === 'SEDOC'" to="/processos/novo" class="btn btn-outline-primary">Novo processo </router-link>
      </div>
      <table class="table table-striped">
        <thead>
          <tr>
            ...
            <th @click="ordenarPor('situacao')" style="cursor:pointer">
              Situação
        ...
        <tbody>
          <tr v-for="processo in processosOrdenados" :key="processo.id" @click="abrirUnidades(processo.id)" style="cursor:pointer">
        ...
        <div class="card h-100">
          <div class="card-body d-flex flex-column justify-content-between">
            ...
            <div v-if="cartao.campos">
              ...
              <router-link v-if="cartao.link" :to="cartao.link.to" class="btn btn-outline-primary btn-sm">{{ cartao.link.rotulo
              }}</router-link>
            </div>
            <!-- Cartões de listas -->
            <ul v-else-if="cartao.chaveLista && cartao.tipo !== 'alertas'" class="list-group list-group-flush">
              <li v-for="item in dadosPainel[cartao.chaveLista].value" :key="item.unidade || item.texto || item.nome"
                ...
                <template v-if="cartao.tipo === 'pendencias'">
                  {{ item.unidade }} <span class="badge" :class="badgeClass(item.situacao)">{{ item.situacao }}</span>
                </template>
                <template v-else-if="cartao.tipo === 'notificacoes'">
                  <span class="badge" :class="badgeClass(item.tipo)">{{ item.tipo === 'info' ? 'Novo' : '!' }}</span>
                  <span class="notification-text ms-2">{{ item.texto }}</span>
                </template>
                <template v-else-if="cartao.tipo === 'processosSubordinadas'">
                  {{ item.nome }} <span class="badge" :class="badgeClass(item.situacao)">{{ item.situacao }}</span>
                </template>
              </li>
            </ul>
            <!-- Tabela de alertas -->
            ...
            <p v-else-if="cartao.tipo === 'situacaoCadastro'">
              ...
              <br>
              <router-link v-if="cartao.link" :to="cartao.link.to" class="btn btn-primary btn-sm mt-2">{{ cartao.link.rotulo
              }}</router-link>
            </p>
          </div>
        </div>
