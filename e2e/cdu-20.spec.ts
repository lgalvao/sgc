...
test.describe.serial('CDU-20 - Analisar validação de mapa de competências', () => {
    ...
    test('1. ADMIN cria processo e CHEFE disponibiliza', async ({page, request}) => {
    ...
    test('2. Aceites e Homologação de Cadastro', async ({page}) => {
    ...
    test('3. ADMIN disponibiliza Mapa', async ({page}) => {
        await login(page, USUARIOS.ADMIN_1_PERFIL.titulo, USUARIOS.ADMIN_1_PERFIL.senha);
        await page.getByTestId('tbl-processos').getByText(descProcesso).first().click();
        await navegarParaSubprocesso(page, UNIDADE_ALVO);
    ...
    test('4. CHEFE valida o mapa', async ({page}) => {
        await login(page, USUARIOS.CHEFE_SECAO_221.titulo, USUARIOS.CHEFE_SECAO_221.senha);
        await acessarSubprocessoChefeDireto(page, descProcesso, UNIDADE_ALVO);
        await navegarParaMapa(page);
    ...
    test('5. GESTOR COORD_22 analisa e aceita', async ({page}) => {
        await login(page, USUARIOS.GESTOR_COORD_22.titulo, USUARIOS.GESTOR_COORD_22.senha);
        await acessarSubprocessoGestor(page, descProcesso, UNIDADE_ALVO);
        await navegarParaMapa(page);
    ...
    test('6. GESTOR SECRETARIA_2 aceita', async ({page}) => {
        await loginComPerfil(page, USUARIOS.CHEFE_SECRETARIA_2.titulo, USUARIOS.CHEFE_SECRETARIA_2.senha, 'GESTOR - SECRETARIA_2');
        await acessarSubprocessoGestor(page, descProcesso, UNIDADE_ALVO);
        await navegarParaMapa(page);
    ...
    test('7. ADMIN homologa final', async ({page, cleanupAutomatico}) => {
