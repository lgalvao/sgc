...
test.describe.serial('CDU-27 - Alterar data limite de subprocesso', () => {
    ...
    test('Setup data', async ({_resetAutomatico, request, page}) => {
        ...
        await adicionarAtividade(page, `Atividade CDU-27 ${timestamp}`);
    });
    ...
    test('Cenario 3: ADMIN altera data limite e recebe confirmação', async ({
        ...
        await verificarToast(page, new RegExp(`${TEXTOS.subprocesso.SUCESSO_DATA_ALTERADA}.*${d}/${m}/${y}`));
