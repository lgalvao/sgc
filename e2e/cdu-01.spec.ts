...
test.describe('CDU-01 - Realizar login e exibir estrutura das telas', () => {
    ...
    test('Deve exibir barra de navegação após login como ADMIN', async ({page}) => {
    ...
    test('Deve exibir barra de navegação com restrições para GESTOR', async ({page}) => {
        await expect(page.getByRole('link', {name: 'SGC'})).toBeVisible();
        await expect(page.getByText('Painel')).toBeVisible();

        // Não-admin deve ver 'Minha unidade' em vez de 'Unidades'
        await expect(page.getByRole('link', {name: 'Minha unidade'})).toBeVisible();
        await expect(page.getByRole('link', {name: 'Unidades'})).toBeHidden();

        // Não-admin não deve ver configurações administrativas
        await expect(page.getByTestId('btn-parametros')).toBeHidden();
        await expect(page.getByTestId('btn-administradores')).toBeHidden();
    });

    test('Deve exibir barra de navegação com restrições para CHEFE', async ({page}) => {
        await expect(page.getByRole('link', {name: 'SGC'})).toBeVisible();
        await expect(page.getByText('Painel')).toBeVisible();
    ...
    test('Deve exibir barra de navegação com restrições para SERVIDOR', async ({page}) => {
        await expect(page.getByRole('link', {name: 'SGC'})).toBeVisible();
        await expect(page.getByText('Painel')).toBeVisible();
