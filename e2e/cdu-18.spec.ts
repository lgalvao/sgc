...
test.describe('CDU-18: Visualizar mapa de competências', () => {
    ...
    test('Cenário 1: ADMIN visualiza mapa via detalhes do processo', async ({
        ...
        await test.step('3. Selecionar unidade de assessoria participante', async () => {
            ...
            await expect(page).toHaveURL(/\/processo\/\d+\/ASSESSORIA_(?:\d+|12)(?:\?.*)?$/);
        });
        ...
        await test.step('5. Verificar visualização do mapa (CDU-18)', async () => {
            ...
            await expect(page.getByText(/Conhecimento Seed 1.1/i)).toBeVisible();
            await expect(page.getByText(/Conhecimento Seed 2.1/i)).toBeVisible();
        });
    });
