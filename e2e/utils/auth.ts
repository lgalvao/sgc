...
export async function loginAsGestor(page: Page) {
    ...
    // Preencher usuário e senha (usa o servidor 2 - Carlos Henrique Lima que tem perfil GESTOR)
    const tituloInput = page.getByTestId('input-titulo');
    await tituloInput.clear();
    await tituloInput.fill('2');
    ...
    if (await seletorDePerfil.isVisible()) {
        // Se o seletor apareceu, escolhe GESTOR para os testes
        await seletorDePerfil.selectOption({label: 'GESTOR - SGP'});
        await page.getByTestId('botao-entrar').click();
    }
