...
test.describe('CDU-03 - Manter Processo', () => {
    ...
    test('Deve validar campos obrigatórios', async ({ page }) => {
        ...
        await page.getByTestId('input-descricao').fill('Descrição Teste');
        await page.getByTestId('btn-salvar').click();
    ...
    test('Deve editar um processo existente', async ({ page }) => {
        ...
        await expect(page.getByTestId('chk-ASSESSORIA_11')).toBeChecked();
    ...
    test('Deve remover um processo', async ({ page }) => {
        ...
        await expect(page).toHaveURL(/\/painel/);
        await expect(page.getByTestId('tabela-processos').getByText(descricao)).not.toBeVisible();
    });
});
