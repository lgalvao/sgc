Empty states para relatórios.
Alertas duplicados para interoperacional

Limpeza de processos:
   - deve apagar notificações tambem
   - melhorar feedback ao concluir:
     - ```Confirma a exclusão completa do processo "Mapeamento da COSIS" (código 497) e de todos os seus dependentes?```
     - ```
       Selecione
       Selecione um processo
       ```

Atualizar notificações ao navegar para a view

Tempo para carregar dados de unidades. Exemplo:
- https://seseldev05.tre-pe.gov.br/processo/498/SEDESENV?codSubprocesso=1007

Email internamente duplicado:

```
    Prezado(a) responsável pela STIC,
    
    Comunicamos o início do processo Mapeamento da COSIS para a sua unidade.
    
    Já é possível realizar o cadastro de atividades e conhecimentos no Sistema de Gestão de Competências (SGC).
    
    O prazo para conclusão desta etapa do processo é 08/08/2026.
    
    Comunicamos o início do processo Mapeamento da COSIS nas unidades SEDESENV, SEDIA, SESEL.
    
    Estas unidades já podem iniciar o cadastro de atividades e conhecimentos. À medida que estes cadastros forem sendo disponibilizados, será possível visualizar e realizar a sua validação.
    
    O prazo para conclusão desta etapa do processo é 08/08/2026.
    
    Acompanhe o processo no Sistema de Gestão de Competências: https://sgc.tre-pe.jus.br.
``` 